...
export function BettingMarkets({ onPlaceBet, filterCategory }: BettingMarketsProps) {
  ...
  const MarketCard = ({ market }: { market: BettingMarket }) => {
    ...
    return (
      <motion.div
        ...
        className="group"
      >
        <Card className="p-4 bg-card border border-border transition-all duration-300 hover:shadow-lg">
          {/* Header */}
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                {market.isHot && (
                  <Badge className="bg-[#F92672]/20 text-[#F92672] border-[#F92672]/30 text-xs">
                    <Flame className="h-3 w-3 mr-1" />
                    Hot
                  </Badge>
                )}
                <Badge variant="outline" className="border-[#66D9EF]/30 text-[#66D9EF] text-xs">
                  {market.cardCategory}
                </Badge>
                <Badge variant="outline" className="border-[#E6DB74]/30 text-[#E6DB74] text-xs">
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      ...
      >
        <Card className="p-3 bg-gradient-to-br from-[#A6E22E]/20 to-[#A6E22E]/10 border border-[#A6E22E]/30">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xl font-bold text-[#A6E22E]">
                {formatNumber(mockMarkets.reduce((sum, m) => sum + m.totalPot, 0))}
              </div>
              <div className="text-xs text-muted-foreground">Total Pot Value</div>
            </div>
            <DollarSign className="h-6 w-6 text-[#A6E22E]" />
          </div>
        </Card>

        <Card className="p-3 bg-gradient-to-br from-[#66D9EF]/20 to-[#66D9EF]/10 border border-[#66D9EF]/30">
          <div className="flex items-center justify-between">
            <div>
            ...
            <Users className="h-6 w-6 text-[#66D9EF]" />
          </div>
        </Card>

        <Card className="p-3 bg-gradient-to-br from-[#F92672]/20 to-[#F92672]/10 border border-[#F92672]/30">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xl font-bold text-[#F92672]">
                {mockMarkets.length}
              </div>
              <div className="text-xs text-muted-foreground">Live Markets</div>
            </div>
            <BarChart3 className="h-6 w-6 text-[#F92672]" />
          </div>
        </Card>

        <Card className="p-3 bg-gradient-to-br from-[#E6DB74]/20 to-[#E6DB74]/10 border border-[#E6DB74]/30">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xl font-bold text-[#E6DB74]">
                {formatNumber(mockMarkets.reduce((sum, m) => sum + m.volume24h, 0))}
              </div>
              <div className="text-xs text-muted-foreground">24h Volume</div>
            </div>
            <TrendingUp className="h-6 w-6 text-[#E6DB74]" />
          </div>
        </Card>
      </motion.div>
        ...
        <div className="flex items-center justify-between mt-0">
          <div className="flex items-center gap-3">
            <select
              ...
              className="px-3 py-2 bg-card border border-border rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-info text-sm"
            >
