...
if (mockDatabase.notifications.size === 0) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all notifications for user (return mock notifications for any user)
    let userNotifications = Array.from(mockDatabase.notifications.values())
      .filter(notification => notification.userId === userId || notification.userId === 'user_pro_001')
    ...
    switch (action) {
      ...
      case 'create':
        ...
        if (notification.requiresSubscription) {
          // Simplified subscription check - assume all users can receive notifications
          const isActive = true
          ...
          // Simplified - allow all notification types for now
        }
