...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'POST':
      try {
        const syncedCard = await nftAPIService.syncNFTOwnership(cardId);
        
        return res.status(200).json({
          message: 'NFT ownership synced successfully',
          card: syncedCard
        });
      } catch (error: any) {
        ...
        return res.status(500).json({ error: 'Failed to sync NFT ownership' });
      }
